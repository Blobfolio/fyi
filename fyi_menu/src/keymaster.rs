...
impl KeyMaster {
	...
	pub fn get2(&self, short: &str, long: &str) -> Option<usize> {
		let mut res = self.keys.eq(u64x8::splat(hash64(short.as_bytes()))).bitmask().trailing_zeros();
		if res < 8 {
			return Some(unsafe { self.values.extract_unchecked(res as usize) });
		}

		res = self.keys.eq(u64x8::splat(hash64(long.as_bytes()))).bitmask().trailing_zeros();
		if res < 8 {
			Some(unsafe { self.values.extract_unchecked(res as usize) })
		}
		else { None }
