...
impl MsgBuffer {
	...
	///
	/// unsafe {
	///     let mut buf = MsgBuffer::from_raw_parts(Vec::new(), [0; 32]);
	///
...
impl MsgBuffer {
	...
	///
	/// Undefined things will happen if `idx` is out of bounds.
	pub const unsafe fn start_unchecked(&self, idx: usize) -> u16 {
		self.toc[idx * 2]
	}
	...
	fn decrease(&mut self, idx: usize, adj: u16) {
		unsafe {
			(
				u16x32::from_slice_unaligned_unchecked(&self.toc) -
				adjustment_mask(idx, adj)
			).write_to_slice_unaligned_unchecked(&mut self.toc);
		}
	}
