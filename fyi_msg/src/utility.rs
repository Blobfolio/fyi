...
/// # Fast Concat
///
/// This quickly concatenates two byte slices into a vector, returning both it
/// and the length total (as a `u16`).
///
/// This is equivalent to `[&[u8], &[u8]].concat()`, but faster owing to the
/// fixed length.
///
/// ## Panics
///
/// Because the length is returned as a `u16` rather than a `usize`, the total
/// bytes may not exceed 65,535.
pub fn concat_slice2(src: [&[u8]; 2]) -> (Vec<u8>, u16) {
	let (a, b) = (src[0].len(), src[1].len());
	let len: usize = a + b;
	let mut v: Vec<u8> = Vec::with_capacity(len);

	unsafe {
		let ptr = v.as_mut_ptr();
		std::ptr::copy_nonoverlapping(src[0].as_ptr(), ptr, a);
		std::ptr::copy_nonoverlapping(src[1].as_ptr(), ptr.add(a), b);
		v.set_len(len);
	}
...
/// # Fast Concat
///
/// This quickly concatenates three byte slices into a vector, returning both
/// it and the total length (as a `u16`).
///
/// This is equivalent to `[&[u8], &[u8], &[u8]].concat()`, but faster owing to
/// the fixed length.
///
/// ## Panics
///
/// Because the length is returned as a `u16` rather than a `usize`, the total
/// bytes may not exceed 65,535.
pub fn concat_slice3(src: [&[u8]; 3]) -> (Vec<u8>, u16) {
	let len: usize = src[0].len() + src[1].len() + src[2].len();
	let mut v: Vec<u8> = Vec::with_capacity(len);
...
/// # Fast Concat
///
/// This quickly concatenates four byte slices into a vector, returning both
/// it and the total length (as a `u16`).
///
/// This is equivalent to `[&[u8]; 4].concat()`, but faster owing to the fixed
/// length.
///
/// ## Panics
///
/// Because the length is returned as a `u16` rather than a `usize`, the total
/// bytes may not exceed 65,535.
pub fn concat_slice4(src: [&[u8]; 4]) -> (Vec<u8>, u16) {
	let len: usize = src[0].len() + src[1].len() + src[2].len() + src[3].len();
	let mut v: Vec<u8> = Vec::with_capacity(len);
...
/// # Fast Concat
///
/// This quickly concatenates five byte slices into a vector, returning both
/// it and the total length (as a `u16`).
///
/// This is equivalent to `[&[u8]; 5].concat()`, but faster owing to the fixed
/// length.
///
/// ## Panics
///
/// Because the length is returned as a `u16` rather than a `usize`, the total
/// bytes may not exceed 65,535.
pub fn concat_slice5(src: [&[u8]; 5]) -> (Vec<u8>, u16) {
	let len: usize = src[0].len() + src[1].len() + src[2].len() + src[3].len() + src[4].len();
	let mut v: Vec<u8> = Vec::with_capacity(len);
...
/// # Fast Concat
///
/// This quickly concatenates six byte slices into a vector, returning both it
/// and the total length (as a `u16`).
///
/// This is equivalent to `[&[u8]; 6].concat()`, but faster owing to the fixed
/// length.
///
/// ## Panics
///
/// Because the length is returned as a `u16` rather than a `usize`, the total
/// bytes may not exceed 65,535.
pub fn concat_slice6(src: [&[u8]; 6]) -> (Vec<u8>, u16) {
	let len: usize = src[0].len() + src[1].len() + src[2].len() + src[3].len() + src[4].len() + src[5].len();
	let mut v: Vec<u8> = Vec::with_capacity(len);
...
mod tests {
	...
	fn t_concat_slice() {
		assert_eq!(
			concat_slice2([b"one", b"two"]),
			(b"onetwo".to_vec(), 6_u16)
		);

		assert_eq!(
			concat_slice3([b"one", b"two", b"three"]),
			(b"onetwothree".to_vec(), 11_u16)
		);

		assert_eq!(
			concat_slice4([b"one", b"two", b"three", b"four"]),
			(b"onetwothreefour".to_vec(), 15_u16)
		);

		assert_eq!(
			concat_slice5([b"one", b"two", b"three", b"four", b"five"]),
			(b"onetwothreefourfive".to_vec(), 19_u16)
		);

		assert_eq!(
			concat_slice6([b"one", b"two", b"three", b"four", b"five", b"six"]),
			(b"onetwothreefourfivesix".to_vec(), 22_u16)
		);

		assert_eq!(
			concat_slice6([b"one", b"two", b"three", b"four", b"", b"six"]),
			(b"onetwothreefoursix".to_vec(), 18_u16)
		);
	}
