...
benches!(
	Bench::new("fyi_msg::MsgBuffer3", "extend(+6)")
		.with(|| {
	...
	Bench::new("fyi_msg::MsgBuffer3", "replace(bigger)")
		.with(|| {
			let mut msg = test_data();
	...
	Bench::new("fyi_msg::MsgBuffer3", "replace(smaller)")
		.with(|| {
			let mut msg = test_data();
			msg.replace(1, b"!!");
			msg
	...
	Bench::new("fyi_msg::MsgBuffer3", "truncate(0)")
		.with(|| {
			let mut msg = test_data();
			msg.truncate(1, 0);
			msg
